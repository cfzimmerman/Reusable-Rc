...
mod tests {
    use super::ReuRcPool;
    use crate::reu_rc::{PoolSizeLimit, ReuRc, ReuRcInner};
    ...
    */
    ...
    fn simple_rc() {
        ...
        let rr4 = rr3.clone();
        drop(rr2);
        let reu_rc_sl = reu_rc.borrow();

        assert_eq!(reu_rc_sl, slice);
        assert_eq!(reu_rc_sl, rr3.borrow());
        assert_eq!(reu_rc_sl, rr4.borrow());
    }
        ...
        complex_access(|pool, maps, pool_max_objs, expect_entries| {
            ...
            for map in maps.iter() {
                for buf in map.values() {
                    assert_eq!(buf.borrow(), &expect_entries);
                }
            }
